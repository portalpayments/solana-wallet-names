export const MIKES_WALLET = "5FHwkrdxntdK24hgQU8qgBjn35Y1zwhz1GZwCkP2UJnM";
export const VIDORS_WALLET = "Hp8SsZZZot8UB28HTfEuxxCXECoSiwHfpzqwenjrMPKF";
export const KRISPYS_WALLET = "FAprSqAv1mKmjLa2VU4pLstrzCQ51x3XsmDBJNXMnkGy";
export const ARMANIS_WALLET = "DcpYXJsWBgkV6kck4a7cWBg6B4epPeFRCMZJjxudGKh4";
export const ALEKSEIS_WALLET = "GXwCSk2RPHDFHPzBSo42CSKU1D94FNoo6qpbZFQG8TfK";
export const WALLET_WITH_NO_NAME =
  "Adyu2gX2zmLmHbgAoiXe2n4egp6x8PS7EFAqcFvhqahz";
